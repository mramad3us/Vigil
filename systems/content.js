...
function generateSourceCode() {
  ...
  return (
    pick(prefixes) + "-" + pick(PERSONNEL_ALIASES) + "-" + randInt(100, 999)
  );
}
...
function generateRandomLocation() {
  ...
  while (attempts < 20) {
    ...
    for (var i = 0; i < theater.cities.length; i++) {
      if (theater.cities[i].country !== "United States")
        eligible.push(theater.cities[i]);
    }
  ...
  return {
    ...
    maritime: fallbackCity.maritime || false,
  };
}
...
function generateIntelValue(
  ...
  if (!pool || pool.length === 0)
    return "Collection in progress — data insufficient.";
